...
export default function App() {
  ...
  /* On load: restore an existing Supabase Auth session, if any, so a
     refresh doesn't log the member out. */
  useEffect(() => {
    (async () => {
      try {
        ...
        // No session, or member row not found yet — stay on login.
      }
    })();
  }, []);
  ...
  useEffect(() => {
    if (surface !== "admin" || !adminAuthed) return;
    let cancelled = false;
    (async () => {
      try {
      ...
      } catch (e) { /* ignore — badge just won't update */ }
    })();
    return () => { cancelled = true; };
  }, [surface, adminAuthed, aTab]);
  ...
  return (
    <div className="sv-root">
      ...
      <div className="dev-bar">
        ...
        <div className="seg">
          <button className={surface === "customer" ? "on" : ""} onClick={() => setSurface("customer")}>
            📱 Customer App
          </button>
          <button className={surface === "admin" ? "on" : ""} onClick={() => setSurface("admin")}>
            ⌨️ Cashier Panel {pendingCount > 0 && <span className="dot-badge">{pendingCount}</span>}
          </button>
        </div>
      </div>
